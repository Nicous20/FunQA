    ...
    for n, (video, video_ids) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        for image_id, topk_id, topk_prob in zip(video_ids, topk_ids, topk_probs):
            ...
            if test_submit:
                ...
                result.append({"id": image_id, "pred_caption": ans})
            else:
...
@torch.no_grad()
def cal_metric(result_file, ref_path):
    bleu = BLEUScore()
    ...
    for index, item in enumerate(result_list):
        predicts.append(item['pred_caption'])
    ref_data = json.load(open(ref_path))
    for index, item in enumerate(ref_data):
        if "Why is the whole video" in item['instruction']:
        #if "Description of the videos" in item['instruction']:
            ref_caps.append(item['output'])
        #bleu_temp=bleu(item['enCap'],[predicts[index]])
            #target=item['enCap']
            #preds=[predicts[index]]
            #print(preds)
            #print(target)
    print(len(predicts))
    print(len(ref_caps))
    for index,item in enumerate(predicts):
        preds=[item]
        target=[ref_caps[index]]
        print(preds)
        print(target)
        bleu_temp=bleu_score(preds, target)
        print(bleu_temp)
        bleu_final.append(bleu_temp)
    #print(predicts)
    #print(ref_caps)
    #result=bleu(predicts, ref_caps)
    result=sum(bleu_final)/len(bleu_final)
...
def main(args, config):
    ...
    test_loader = create_loader(datasets, samplers,
                                ...
                                num_workers=[1], is_trains=[False], collate_fns=[nocaps_collate_fn])[0]
    ...
    if args.checkpoint:
        ...
        if config["clip_name"] == "ViT-B/32":
            num_patches = int(config["image_res"] * config["image_res"] / (32 * 32))
    ...
    for k, v in submission.items():
        ...
        elif 'a creative video because of' in v:
            v = v.split('a creative video because of')[-1]
        new_submission[k] = v
    ...
    # offline evaluation
    result_file_path = os.path.join(args.result_dir, '%s_rank%d.json'%('vatex_video_caps_test',0))
    #vid_refs_list = proces_res_file(result_file_path, config['ref_path'])
    #result_file = os.path.join(args.result_dir, 'result.json')
    #json.dump(vid_refs_list, open(result_file, 'w'))
    metirc = cal_metric(result_file_path, config['ref_path'])
    print('bleu score', metirc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/videocap_vatex_mplug_large.yaml')
    ...
    parser.add_argument('--do_amp', action='store_false')
